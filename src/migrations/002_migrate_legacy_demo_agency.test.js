...
describe("002_migrate_legacy_demo_agency", () => {
  it("creates the full entity graph and migrates every legacy shift, preserving ids", async () => {
    ...
    // Two Organisation rows exist against Example Driver Agency's own workspace
    // after migration 006 runs: Example Driver Agency's own self-org, and a
    // proper client Organisation for Example Logistics (previously just a free-text
    ...
    const clientOrg = orgsInWorkspace.find((o) => o.tradingName === "Example Logistics");
    ...
    expect(site.name).toMatch(/Example Logistics/);
    // Site.organisationId points at the CLIENT (Example Logistics), not the agency's own org.
    ...
    expect(rateCard.rates.MonThu.Days).toEqual([12.00, 14.00]);
    ...
    // providerOrganisationId (Stage 4D) is Example Driver Agency's own self-org — who employs Alex, not the client he's placed with.
