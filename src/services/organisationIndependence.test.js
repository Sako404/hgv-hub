...
describe("Organisation/Site independence across agency workspaces", () => {
  it("the same client name + site name can carry a different Rate Card under two different agencies", async () => {
    ...
    const clientUnderA = await createOrganisation({ workspaceId: agencyAWorkspaceId, legalName: "Example Logistics", tradingName: "Example Logistics", types: ["client"] }, db);
    const siteUnderA = await createSite({ organisationId: clientUnderA.id, name: "Depot A Depot" }, db);
    const rateCardA = await createRateCard(
      { workspaceId: agencyAWorkspaceId, name: "Agency A — Example Logistics Depot A", effectiveFrom: "2026-01-01", rates: RATES_A },
      db
    );
    ...
    const clientUnderB = await createOrganisation({ workspaceId: agencyBWorkspaceId, legalName: "Example Logistics", tradingName: "Example Logistics", types: ["client"] }, db);
    const siteUnderB = await createSite({ organisationId: clientUnderB.id, name: "Depot A Depot" }, db);
    const rateCardB = await createRateCard(
      { workspaceId: agencyBWorkspaceId, name: "Agency B — Example Logistics Depot A", effectiveFrom: "2026-01-01", rates: RATES_B },
      db
    );

    // Distinct records throughout — no shared/global "Example Logistics" or "Depot A" entity.
