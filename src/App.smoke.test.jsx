...
function mainContent() {
  ...
  // workspace switcher — whose <option> labels (e.g. "Apex Driving")
...
describe("AppShell — driver experience", () => {
  ...
  it("boots straight into the driver sidebar for Alex, with no workspace switcher", async () => {
    ...
    // Driver-only membership in Apex Driving must NOT trigger the switcher.
    ...
    expect(screen.getByText(/Apex Driving/)).toBeInTheDocument();
  });
  ...
  it("Stage 4E: Add Shift picker defaults to the last-used assignment, not always the first one", async () => {
    ...
    // gives him one at Apex Driving/Parcel Line) so the picker actually
  ...
  it("Add Shift (PL-2): a per-load assignment shows the Loads editor, prices from entered loads, and round-trips through edit", async () => {
    // A second, per-load assignment alongside Alex's real Apex
    // Driving/Parcel Line (hourly) one, set up BEFORE render — same lesson as
  ...
  it("Workplaces: add a second workplace, set it as the explicit default, and see it reflected in the Add Shift picker", async () => {
    const db = await bootAppWithLegacyData(); // Alex, one active workplace already (Apex Driving/Parcel Line)
    const user = userEvent.setup();
    ...
    // in the subtitle line below ALSO contains "Apex Driving"/"Parcel Line".
    expect(await within(mainContent()).findByText("Apex Driving · Parcel Line Northfield")).toBeInTheDocument();
...
describe("AppShell — company experience and workspace switching", () => {
  ...
  it("Management group: Organisations and Sites are navigable and reflect the real corrected data", async () => {
    ...
    // Alex's own membership in Apex Driving is driver-only — give him
    ...
    const switcher = await screen.findByLabelText("Switch workspace");
    const user = userEvent.setup();
    await user.selectOptions(switcher, screen.getByText("Apex Driving"));
    ...
    // Apex Driving's own self-org and the corrected Parcel Line client org both show up.
    expect(await within(mainContent()).findByText("Apex Driving")).toBeInTheDocument();
    expect(within(mainContent()).getByText("Parcel Line")).toBeInTheDocument();

    await user.click(within(nav).getByText("Sites"));
    expect(await screen.findByRole("heading", { name: "Sites" })).toBeInTheDocument();
    expect(await within(mainContent()).findByText(/Parcel Line Northfield/)).toBeInTheDocument();
    // Site now correctly shows the CLIENT org (Parcel Line), not Apex Driving's own org.
    expect(within(mainContent()).getByText(/Parcel Line · Client site/)).toBeInTheDocument();
  });
  ...
  it("Add Organisation end to end: form -> service -> list refresh, via the real UI", async () => {
    ...
    render(<App />);
    const user = userEvent.setup();
    await user.selectOptions(await screen.findByLabelText("Switch workspace"), screen.getByText("Apex Driving"));
  ...
  it("Rate Cards: list shows the real Apex/Parcel Line lineage; create, revise, and archive all work end to end", async () => {
    const db = await createIndexedDbDb();
    await runMigrations(db, window.localStorage);
    window.localStorage.setItem(STORAGE_KEYS.CURRENT_PERSON_ID, "person-demo");
    const membership = (await db.memberships.query({ where: { workspaceId: "workspace-demo-agency", personId: "person-demo" } }))[0];
    ...
    // The real, migrated Apex Driving/Parcel Line lineage shows as one card, one version.
    expect(await within(mainContent()).findByText(/Apex Driving.*Parcel Line/)).toBeInTheDocument();
  ...
  it("property 9: a driver-only role never sees the Management group or a Drivers management screen", async () => {
    await bootAppWithLegacyData(); // Alex, driver-only in Apex Driving, no manager-tier role anywhere
    expect(screen.queryByLabelText("Switch workspace")).not.toBeInTheDocument();
  ...
  it("Add Driver end to end: create, edit, archive, and restore, all through the real UI with real persistence", async () => {
    ...
    // for Apex Driving yet — absence means active) — scope to this driver's card.
    await waitFor(() => {
  ...
  it("Work Placements: create, assign a driver, end, and archive all work end to end through the real UI", async () => {
    ...
    await user.click(within(mainContent()).getByText(/Apex Driving.*Parcel Line Northfield/));
    ...
    await user.selectOptions(providerSelect, within(providerSelect).getByText("Apex Driving"));
    const siteSelect = screen.getByLabelText("Site");
    await user.selectOptions(siteSelect, within(siteSelect).getByText(/Parcel Line Northfield/));
    const rateCardSelect = screen.getByLabelText("Rate card");
    await user.selectOptions(rateCardSelect, within(rateCardSelect).getByText(/Apex Driving/));
    const dateInput = document.querySelector('input[type="date"]');
    await user.clear(dateInput);
    await user.type(dateInput, "2026-01-01");
    await user.click(screen.getByText("Save"));
    ...
    await waitFor(() => {
      expect(within(mainContent()).getAllByText(/Apex Driving.*Parcel Line Northfield/).length).toBe(2);
    });
    ...
    await user.click(within(newCard).getByText(/Apex Driving.*Parcel Line Northfield/));
